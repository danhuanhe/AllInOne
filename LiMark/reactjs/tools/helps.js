...
const modifyNeiServerPort = (neiConfigPath, port) => {
  ...
  fs.readFile(neiConfigPath, 'utf8', (err, content) => {
    ...
    console.log('modifyNeiPort Success, port:', port);
    let result = content.replace(/port:\s?(\d+)/g, 'port: ' + port);
    fs.writeFile(neiConfigPath, result, 'utf8', (err) => {
